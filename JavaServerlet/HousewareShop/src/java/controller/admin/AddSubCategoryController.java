...
public class AddSubCategoryController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        SubCategory subCategory = SubCategory.builder().subCategoryName(name)
                .categoryId(category).status(status).build();
        boolean check = new SubCategoryDao().add(subCategory);
        if(check){
            response.sendRedirect("get-sub-category");
        }else{
            request.setAttribute("error", "Add Sub Category Failed!");
